...
/* Rota para registrar um admin, sendo necessário ser um admin */
...
router.delete("/delete/:nome", verifAdmin, (req, res) => {
  ...
  if (!req.params.nome) {
    return res
      .status(400)
      .json({ message: "O nome do usuário é obrigatório." });
  }

  // Procura o usuario com o nome que foi passado por parametro
  User.findOne({ nome: req.params.nome })
    .then((user) => {
      if (!user) {
      ...
      if (user.isAdmin === 1) {
        return res
          .status(403)
          .json({ message: "Não é permitido excluir um administrador." });
      }

      // Procede com a exclusão do usuário se não for um administrador
      User.deleteOne({ nome: req.params.nome })
        .then((result) => {
...
router.delete("/delete-myself", verifAdmin, (req, res) => {
  ...
  User.deleteOne({ _id: userIdFromToken })
    .then((result) => {
      if (result.deletedCount > 0) {
